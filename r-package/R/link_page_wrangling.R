...
prepare_page_name <- function(course_axis){
  ...
  page_name <- course_axis %>%
    ...
    dplyr::filter(grepl("Quiz",name)==FALSE) %>%
    dplyr::filter(grepl("Question",name)==FALSE) %>%
    dplyr::mutate(path = substr(path,1,66))

  # Get a module name vector
  chap_name <- as.vector((page_name %>%
                            dplyr::arrange(index) %>%
                            dplyr::filter(category == "chapter") %>%
                            dplyr::select(name))$name)

  # Get module index
  chap_index <- as.vector((page_name %>%
                             dplyr::arrange(index) %>%
                             dplyr::filter(category == "chapter") %>%
                             dplyr::select(index))$index)
...
write_page_clean <- function(input_course,cleaned_data){
  ...
  dplyr::write_csv(x=cleaned_data, path= output_csv_path)
