...
#' @description Reads the raw .csv obtained through rbq.py into a dataframe. For documentation on how to use
...
write_wrangled_video_data <- function(input_course, cleaned_data, testing=FALSE){
  ...
  # Save data frame to csv. This data frame is used 
  # for the video heat map visualization. 
...
prepare_video_data <- function(video_data, video_axis){
  ...
  # Obtain the max time recorded for each stop_video event by
  # assuming the mode stop time is the maximum video length.
  # This is better than the video_length table available on
  # big query which has some inaccurate values
  # (Ex :3 day long video lengths)
  ...
  # Place max video length column into dataframe
  # This will be used as an integrity check
  ...
  # Place course order column into dataframe
  # This is how videos are ordered in the final plot
...
#' @description Parses dataframe and adds columns \code{start} and \code{end} showing
...
get_start_end_df <- function(data){
  start_end_df <- data %>% 
    ...
    mutate(stop_event = (event_type != 'speed_change_video' &
                           event_type != 'seek_video')) %>% 
    group_by(username, event_group) %>% 
    mutate(stop_events = cumsum(stop_event)) %>% 
    dplyr::filter(stop_events <= 2) %>% 
    mutate(event_type_next = lead(event_type),
    ...
    ungroup()  %>% 
    mutate(event_type_next = ifelse(is.na(event_type_next), 'DONE', event_type_next)) %>% 
    mutate(start = case_when(.$event_type == "play_video" ~ .$position,
                           .$event_type == "seek_video" ~.$new_time,
                           .$event_type == "speed_change_video" ~ .$speed_change_position,
                           TRUE ~ as.double(NA))) %>% 
    mutate(end = case_when(.$event_type_next == "pause_video" ~ .$position_next,
                           .$event_type_next == "seek_video" ~ .$old_time_next,
                           .$event_type_next == "speed_change_video" ~ .$speed_change_position_next,
                           .$event_type_next == "page_close" ~ get_end_time(.$start, .$time, .$time_ahead, .$latest_speed),
                           .$event_type_next == "seq_next" ~ get_end_time(.$start, .$time, .$time_ahead, .$latest_speed),
                           .$event_type_next == "seq_prev" ~ get_end_time(.$start, .$time, .$time_ahead, .$latest_speed),
                           stringr::str_detect(.$event_type_next, "problem") ~ get_end_time(.$start, .$time, .$time_ahead, .$latest_speed),
    ...
    select(video_id, video_name, 
           mode, certified, gender, 
           activity_level, max_stop_position, 
           course_order, index_chapter, 
           chapter=chapter_name, username,
           start, end)
...
get_watched_segments <- function(data) {
  #View(data)
  # Get maximum time watched for all videos:
  ...
  for (j in 0:(num_segs-1)){
    ...
    data[,start_segment_column+j] <- ifelse((data$start<=segment_low & data$end>=segment_high) | 
                                              (data$start>=segment_low & data$start<=segment_high) | 
                                              (data$end>=segment_low & data$end<=segment_high), 
                                            1, 
                                            0)
  }
...
make_tidy_segments <- function(data){
  ...
  tidy_segment_df <- data %>%
    gather(key=segment, value=count, (start_segment_column):ncol(data)) %>%
  ...
  tidy_segment_df <- tidy_segment_df %>% 
    dplyr::filter(count > 0) %>% 
    dplyr::filter(is.na(username) == FALSE) %>% 
    select(-start, -end, -count, -segment)
  
  tidy_segment_df <- tidy_segment_df %>%
    group_by(video_id, username, min_into_video, video_name, mode, certified, gender, 
             activity_level, max_stop_position, course_order, index_chapter, chapter) %>%
    summarize(count = n())
...
#'
#' @return \code{integrity}: Boolean of whether or not the data passes integrity checks
#'
#' @examples
...
check_integrity <- function(start, end, max_stop_position){
  ...
  all_checks <- !(na_check | passed_ending_check | start_larger_than_end_check | watch_duration_check)
...
#' Outputs cleaned video data into corresponding course directory
#' @description This function will automatically read a file named "generalized_video_heat.csv"
...
wrangle_video <- function(input_course, testing=FALSE){
  ...
  # Add segment columns to dataframe to see whether or not 
  # the segment is within the start and end times
  ...
  # Write data:
  write_wrangled_video_data(input_course=input_course, cleaned_data=tidy_segment_df, testing)
}