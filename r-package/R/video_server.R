...
get_aggregated_df <- function(filt_segs, top_selection){
  ...
  # Getting dataframe with number of unique user views of videos
  # Note: This still counts scenarios if a person watches a video for 0.125 seconds
  #       No thresholding has been conducted.
  ...
  # Only select segments within videos that have been watched at least once
  # This is the data frame that is used to make plots:
  ...
  aggregate_segment_df <- aggregate_segment_df %>%
    left_join(avg_watch_rate_df, by='video_id') %>% 
  ...
  model_df <- data.frame(predict(model, interval="confidence")) %>% 
    ...
    mutate(top_surprise = case_when(.$top_positive == 1 ~ 1,
                                    .$top_negative == 1 ~ 2,
                                    TRUE ~ 0))
  
  # Adding positive and negative residual ranks to dataframe:
  aggregate_segment_df <- aggregate_segment_df %>% 
    mutate(top_surprise = model_df$top_surprise) 
  
  up_until_df <- filt_segs %>% 
    group_by(video_id, username) %>% 
...
get_ch_markers  <- function(filt_segs){
  ...
  ch_markers <- unique(video_ch_markers$ch_marker[!is.na(video_ch_markers$ch_marker)]) - 0.5
  return(ch_markers)
}
...
get_video_lengths <- function(filt_segs){
  vid_lengths <- filt_segs %>% 
    group_by(video_id) %>% 
    summarise(`Video Length` = round(unique(max_stop_position/SECONDS_IN_MINUTE),2))
...
get_summary_table <- function(aggregate_df, vid_lengths){
  summary_tbl <- aggregate_df %>% 
    ungroup() %>% 
    group_by(video_id) %>% 
    summarize(`Average Views per Student` = mean(watch_rate),
              `Students` = unique(unique_views),
              course_order = max(course_order),
              video_name = unique(video_name)) %>% 
    mutate(`Average Views per Student` = round(`Average Views per Student`, 2)) %>% 
...
get_top_hotspots_plot <- function(filtered_segments, module, filtered_ch_markers){
  g <- ggplot(filtered_segments, 
         aes_string(fill="top_watch_rate",
                    x="min_into_video",
                    y="video_id",
                    text='paste0(video_name, "<br>",
                    watch_rate, " views per student", "<br>",
...
get_top_surprise_plot <- function(filtered_segments, module, filtered_ch_markers){
  g <- ggplot(filtered_segments, 
              aes_string(fill="top_surprise",
                         x="min_into_video",
                         y="video_id",
                         text='paste0(video_name, "<br>",
                         watch_rate, " views per student", "<br>",
