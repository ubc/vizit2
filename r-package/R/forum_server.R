...
count_authors <- function(input_forum, wrangled_forum_elements){
        ...
        author_counts <- right_join(wrangled_forum_elements, author_counts)
...
count_views <- function(input_forum, wrangled_forum_elements) {
        ...
        view_counts <- right_join(wrangled_forum_elements, view_counts)
...
get_subcategory_options <- function(category,
        ...
        if (category == "All") {
                ...
                subcategory_options <- append(subcategory_options,
                                              as.character(unique(filtered_forum_elements$discussion_category)))
...
update_forum_data <- function(forum_posts,
        ...
        withProgress(message = "Filtering forum data.", value = 0, {
                ...
                updated_forum_data <- left_join(forum_elements, joined_forum_data)
                
                # Set NA values to 0.
                updated_forum_data$posts[is.na(updated_forum_data$posts)] <- 0
                updated_forum_data$Discussion[is.na(updated_forum_data$Discussion)] <- 0
                updated_forum_data$Question[is.na(updated_forum_data$Question)] <- 0
                updated_forum_data$Response[is.na(updated_forum_data$Response)] <- 0
                updated_forum_data$Comment[is.na(updated_forum_data$Comment)] <- 0
                updated_forum_data$authors[is.na(updated_forum_data$authors)] <- 0
                updated_forum_data$views[is.na(updated_forum_data$views)] <- 0
                ...
                if (category == "All") {
                        ...
                        updated_forum_data <- updated_forum_data %>% 
                                ...
                                summarize(course_order = min(course_order),
...
specify_forum_data_selection <- function(updated_forum_data,
        ...
        if (category == "All") {
                ...
                forum_w_selection$selected[forum_w_selection$discussion_category == selected_subcategory] <- "TRUE"
                
        } else {
                
                # If there is a selection match, set it to TRUE.
                forum_w_selection$selected[forum_w_selection$display_name == selected_subcategory] <- "TRUE"
...
get_wordcloud_data <- function(filtered_wordcloud_data,
        ...
        withProgress(message = "Counting words in filtered wordcloud data.", value = 0, {
...
set_wordcloud_title <- function(category,
        ...
        if (category == "All") {
                ...
                if (selected_subcategory == "All") {
                        ...
                        wordcloud_title <- paste0("Most used words in ", tags$em(selected_subcategory))
...
set_forum_threads_title <- function(category,
        ...
        if (category == "All") {
                ...
                if (selected_subcategory == "All") {
                        ...
                        forum_threads_title <- paste0("Threads in ", tags$em(selected_subcategory))
...
set_axis_limit <- function(forum_data,
        ...
        if (max_bar_length != 0) {
                ...
                # (In case there are many longest bars, take the longest label of those bars.)
                label_longest <- max(label_lengths[bar_lengths == max_bar_length])
        ...
        axis_limit <- max_bar_length + (max_bar_length*percent_addition_per_char*label_longest) + min_axis_length
...
get_reactive_xvar_string <- function(category) {
        ...
        if (category == "All") {
                ...
                reactive_xvar_string <- "forcats::fct_reorder(discussion_category, course_order, .desc = TRUE)"
                
                return(reactive_xvar_string)
                
        } else {
                
                reactive_xvar_string <- "forcats::fct_reorder(display_name, course_order, .desc = TRUE)"
...
make_forum_barplot <- function(forum_data,
        ...
        if (breakdown == TRUE) {
                ...
                forum_barplot <- make_forum_breakdown_barplot(gathered = gathered,
...
get_author_count <- function(forum_posts,
        ...
        author_count <- paste0(length(unique(authors$author_id)), " authors")
...
get_viewer_count <- function(forum_views,
        ...
        viewer_count <- paste0(length(unique(viewers$user_id)), " viewers")
