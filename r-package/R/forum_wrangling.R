...
#'
#' @param posts_input_path The path to the CSV file containing the forum posts 
#'   data.
#' @param views_input_path The path to the CSV file containing the forum reads 
#'   data.
#' @param searches_input_path The path to the CSV file containing the forum 
#'   searches data.
#' @param json_input_path The path to the JSON file containing data on all 
#'   course elements.
#' @param xml_input_path The path to the XML file containing data on the course 
#'   structure.
#' @param posts_output_path The desired path to which to write the prepared 
#'   forum posts dataframe.
#' @param words_output_path The desired path to which to write the prepared 
#'   forum words dataframe.
#' @param views_output_path The desired path to which to write the prepared 
#'   forum views dataframe.
#' @param searches_output_path The desired path to which to write the prepared 
#'   forum searches dataframe.
#' @param elements_output_path The desired path to which to write the prepared 
...
wrangle_forum <- function(posts_input_path,
  ...
  wrangled_forum_posts <-
    left_join(forum_posts, wrangled_forum_elements)
  
  # Reorder the variables in wrangled_forum_posts so they're easier to read by a 
  # human.
  ...
  wrangled_forum_words <-
    left_join(forum_words, wrangled_forum_elements)
  ...
  wrangled_forum_views <-
    left_join(forum_views, wrangled_forum_elements)
  ...
  print(
    "Complete. `wrangled_forum_posts`, `wrangled_forum_words`, 
    `wrangled_forum_views`, `wrangled_forum_searches`, and 
    `wrangled_forum_elements` have been saved to csv."
  )
}
...
prepare_posts <- function(forum) {
  ...
  # Rename the post types once more to distinguish between discussions and 
  # questions.
  forum_posts <-
    forum_w_inferred_subcategories %>% mutate(
      type = case_when(
        .$post_type == "initial_post" &
          .$initial_post_type == "discussion" ~ "Discussion",
        .$post_type == "initial_post" &
          .$initial_post_type == "question" ~ "Question",
...
get_post_types <- function(forum) {
  forum_w_post_types <- forum %>% mutate(
    post_type = case_when(
      .$bq_post_type == "CommentThread" ~ "initial_post",
      (.$bq_post_type == "Comment") &
        (is.na(.$parent_id) == TRUE) ~ "response_post",
      (.$bq_post_type == "Comment") &
        (is.na(.$parent_id) == FALSE) ~ "comment_post"
    )
  )
...
infer_post_subcategories <- function(forum) {
  ...
  response_posts_w_sc <- response_posts %>%
    left_join(initial_posts_for_join,
              by = c("comment_thread_id" = "mongoid")) %>%
  ...
  comment_posts_w_sc <- comment_posts %>%
    left_join(response_posts_w_sc_for_join,
              by = c("parent_id" = "mongoid")) %>%
    mutate(commentable_id = commentable_id.y) %>%
    select(-commentable_id.x, -commentable_id.y)
  ...
  comment_counts <- comment_posts_w_sc %>%
    ...
    ungroup() %>% 
    right_join(response_posts_w_sc_for_join,
               by = c("parent_id" = "mongoid")) %>% 
  ...
  response_counts <- response_posts_w_sc %>%
    ...
    ungroup() %>% 
    right_join(initial_posts_for_join,
               by = c("comment_thread_id" = "mongoid")) %>%
    mutate(initial_post_id = comment_thread_id)
...
#' @return A dataframe with four columns: \code{commentable_id}, 
#'   \code{display_name}, \code{discussion_category}, and 
#'   \code{discussion_target}.
#' @examples
#' prepare_json(json)
#' @export
prepare_json <- function(json) {
  ...
  for (i in 1:length(json)) {
    ...
    if ("course" %in% as.character(json[[i]])) {
      ...
      for (name in 1:length(names(json[[i]]$metadata$discussion_topics))) {
        ...
        commentable_id <- append(
          commentable_id,
          as.character(json[[i]]$metadata$discussion_topics[[name]])
        )
        display_name <- append(
          display_name, 
          names(json[[i]]$metadata$discussion_topics[name])
        )
        discussion_category <- append(
          discussion_category,
          names(json[[i]]$metadata$discussion_topics[name])
        )
        discussion_target <- append(
          discussion_target,
          names(json[[i]]$metadata$discussion_topics[name])
        )
      }
  ...
  # As a relic of the JSON parsing, the naming of discussion_id doesn't always 
  # come out right.
...
#' @return A dataframe with three columns: \code{display_name}, 
#'   \code{discussion_category}, and \code{discussion_target}.
#' @examples
#' prepare_xml(xml)
#' @export
prepare_xml <- function(xml) {
  ...
  if (class(all_parameters) == "list") {
    ## Some course XMLs spit out a nested list of discussion node parameters.
    ...
    ## Other course XMLs spit out a matrix of discussion node parameters.
  ...
  current_cat <- forum_elements_xml$discussion_category[1]
  current_cat_lev <- 1
  category_levels <- c()  
  # Initialize a vector to store the order of the discussion categories.
  for (i in 1:dim(forum_elements_xml)[1]) {
    if (is.na(forum_elements_xml$discussion_category[i])) {  
      # Sometimes a discussion category will be NA (e.g. in Creative Writing)
...
#' Get the variables associated with each forum element in the JSON file. (for 
#'   use in \code{prepare_json}).
#'
#' @param i An iterator.
#' @param all_elements A nested list containing attributes for each course 
#'   element.
#' @return A vector containing the \code{commentable_id}, \code{display_name}, 
#'   \code{discussion_category}, and \code{discussion_target} of a course 
#'   element.
#' @examples
...
#' Get the variables associated with each forum element in the XML file (for use 
#'   in \code{prepare_xml}).
#'
#' @param i An iterator.
#' @param all_parameters An XML tree containing all the parameters associated 
#'   with each discussion node.
#' @return A vector containing the \code{display_name}, 
#'   \code{discussion_category}, and \code{discussion_target} of a course 
#'   element.
#' @examples
