...
#'
#' @description Reads the input course's raw 'generalized_demographics.csv' and outputs a cleaned version as wrangled_demographics.csv
...
#' Obtains a data frame with countries and ISO codes of their associated languages
#'
...
#' @param language_info Data frame containing languages and their associated ISO codes
#' @param country_info Data frame with countries and their associated languages as ISO codes
#'
#' @return Cleaned data frame containing demographic information
#'
#' @examples prepare_general_data(obtain_raw_general_data('psyc1'), language_info(), country_info())
prepare_general_data <- function(data, language_info, country_info)
{
  # Add proper activity level:
  data <- data %>% mutate(activity_level = case_when((.$sum_dt < 1800) ~ 
                                                       "under_30_min", (.$sum_dt >= 1800) & (.$sum_dt < 18000) ~ "30_min_to_5_hr", 
                                                     (.$sum_dt >= 18000) ~ "over_5_hr", is.na(.$sum_dt) ~ "NA"))
  
  # Data frame with language code and language name
  language_info <- language_info %>% mutate(primary_lang = alpha2) %>% 
    mutate(language = English) %>% select(primary_lang, language)
  ...
  data <- data %>% mutate(ISO = cc_by_ip) %>% left_join(country_info, 
                                                        by = "ISO") %>% select(-language) %>% left_join(language_info, 
                                                                                                        by = "primary_lang")
