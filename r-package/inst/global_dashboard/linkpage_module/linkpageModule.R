linkpageModule <- function(input, output, session) {
  ...
  output$chap_name_linkpage <- renderUI({
    ...
    selectInput(ns("module"),
                "Module:",
                choices = append("All",
                                 get_module_vector(item_df = page_name())[-1]),
                selected = "All")
  ...
  log_dat_filtered <- reactive({
    ...
    log_dat <- filter_demographics(log_dat(),
                                   gender = input$gender,
                                   mode = input$mode,
  ...
  reactive_page <- reactive({
    ...
    if (nrow(page_student) == 0) {
      ...
      # Join two tables for getting the name of each page, 
      # common column is path (two hash strings)
      page_name_mapping <- inner_join(page_student,
                                      page_name(),
                                      by = "path")
      
      # Filter page by chapter
      page_name_mapping <- filter_chapter_linkpage(page_name_mapping,
                                                   module = input$module)
  ...
  reactive_link_dat <- reactive({
    ...
    link_dat <- filter_demographics(link_dat(), 
                                    gender = input$gender, 
                                    mode = input$mode,
                                    activity_level = input$activity_level)
    
    ## Filter chapter
    link_dat <- filter_chapter_linkpage(link_dat,
                                        module = input$module)
