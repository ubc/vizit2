forumModule <- function(input, output, session) {
  observe({
    ...
    wrangled_forum_posts <- reactive({
      validate(need(try(nrow(wrangled_forum_posts_csv()) > 0)
    ...
    wrangled_forum_words <- reactive({
      validate(need(try(nrow(wrangled_forum_words_csv()) > 0)
                    , "No forum words found."))
      wrangled_forum_words_csv()
    })
    ...
    wrangled_forum_views <- reactive({
      validate(need(try(nrow(wrangled_forum_views_csv()) > 0)
                    , "No forum views found."))
      wrangled_forum_views_csv()
    })
    ...
    wrangled_forum_elements <- reactive({
      validate(need(try(nrow(wrangled_forum_elements_csv()) > 0)
                    , "No forum elements found."))
      wrangled_forum_elements_csv()
    })
    ...
    wrangled_forum_searches <- reactive({
      validate(need(try(nrow(wrangled_forum_searches_csv()) > 0)
                    , "No forum searches found."))
      wrangled_forum_searches_csv()
    })
    ...
    output$category_select <- renderUI({
      ...
      selectInput(
        ns("category"),
        "Category:",
        choices = append(c("All"), unique(
          as.character(wrangled_forum_elements()$discussion_category)
        )),
        selected = "All"
      )
    ...
    forum_searches <- reactive({
      calculate_forum_searches(
        forum_searches = wrangled_forum_searches(),
    ...
    output$subcategory_select <- renderUI({
      ...
      selectInput(
        ns("subcategory"),
        "Subcategory:",
        choices = get_subcategory_options(
          category = category(),
          filtered_forum_elements = filtered_forum_elements()
        ),
        selected = "All"
    ...
    filtered_wordcloud_data <- reactive({
      filter_wordcloud_data(
        forum_words = wrangled_forum_words(),
        forum_elements = wrangled_forum_elements(),
        activity_level = activity_level(),
        gender = gender(),
        registration_status = registration_status(),
    ...
    output$forum_plot_subtitle <- renderText({
      paste0("Selected subcategory: ",
             tags$em(selected_subcategory()))
    })
    ...
    wordcloud_data <- reactive({
      get_wordcloud_data(
        filtered_wordcloud_data = filtered_wordcloud_data(),
        filtered_forum_elements = filtered_forum_elements(),
        category = category(),
        selected_subcategory = selected_subcategory()
      )
    ...
    output$searches <- renderDataTable(# Show five search queries, by default.
      options = list(pageLength = 5),
