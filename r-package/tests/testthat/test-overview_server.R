...
test_that(
  "Proper warning message pops up if dataframe is empty after joining engage table and item table",
  {
    ...
    expect_equal(
      suppressWarnings(join_engagement_item(engage_df, item_df)$nactive[1]) ,
      "Sorry, no data matched this filtering condition"
    )
  }
)
